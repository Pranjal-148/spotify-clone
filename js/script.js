...
async function getSongs(folder) {
    ...
    for (const song of songs) {
        ...
        <li data-song="${song}">
            ...
            <div class="like">
                <img width="24" src="${isLiked(song) ? "img/heart-filled.svg" : "img/heart-empty.svg"}" class="like-icon" title="Like">
            </div>
        </li>`;
    }
    ...
    document.querySelectorAll(".like-icon").forEach(icon => {
        let songName = icon.closest("li").dataset.song;

        icon.addEventListener("click", (e) => {
            e.stopPropagation(); // Prevent playing song on heart click
            toggleLike(songName, icon);
        });
    });
